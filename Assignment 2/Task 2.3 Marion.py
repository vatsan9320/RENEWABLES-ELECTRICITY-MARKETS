...
#### Je rajoute Epsilon en parameter de ALSO-X pour pouvoir calculer avec différentes valeurs
...
nbr_in_sample=100
nbr_seconds=60
#epsilon=np.arange(0,22/100, 2/100)
...
def ALSO_X_relaxed(in_sample, epsilon): #Every binary variable is relaxed between 0 and 1 : it solves several LP until the tolerance is reached
    low_q=0
...
for eps in epsilon:
    ...
    #ALSO_optimal_bid.append(ALSO_results[f"Epsilon={eps}"]["c_up"])
    for i in range(len(out_of_sample_profiles)):
        ALSO_shortfall[f"Epsilon={eps}"]=ALSO_results[f"Epsilon={eps}"]["c_up"]*np.ones(60)-out_of_sample_profiles[f"Profile{i+1}"]
    proba_reserve_shortfall[f"Epsilon={eps}"]=np.sum(np.array(ALSO_shortfall[f"Epsilon={eps}"]) > 0)/len(ALSO_shortfall)/60*100
...
# Axe pour ALSO_results
color1 = 'tab:blue'
ax1.set_xlabel("Epsilon")
ax1.set_ylabel("Optimal Reserve Bid (kW)", color=color1)
ax1.plot(epsilon, [ALSO_results[f"Epsilon={eps}"]["c_up"] for eps in epsilon], label="Optimal Reserve Bid (kW)", color=color1)
ax1.tick_params(axis='y', labelcolor=color1)

# Axe pour proba_reserve_shortfall
ax2 = ax1.twinx()  # Second axe y
color2 = 'tab:orange'
ax2.set_ylabel("Overbidding Probability (%)", color=color2)
ax2.plot(epsilon, [proba_reserve_shortfall[f"Epsilon={eps}"] for eps in epsilon], label="Overbidding Probability", color=color2)
ax2.tick_params(axis='y', labelcolor=color2)

# Ajouter une légende combinée
fig.tight_layout()  # Ajuste le layout pour éviter les chevauchements
plt.show()

print("Je comprends pas les % que j'obtiens, \
      ça n'a aucune sens, les valeurs sont super faibles meme pour un grand epsilon et parfois, sorti de nulle part ça semble fonctionner")